...
//  RubyGems
//
//  Created by Christian Torres on 7/24/21.
//

import UIKit
import CoreData

protocol GemSelectionDelegate: AnyObject {
    func gemSelected(_ newGem: Gem)
}

class ListTableViewController: UITableViewController {
    
    weak var delegate: GemSelectionDelegate?
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.mainColorLbl.textColor = UIColor(named: gem.mainColor!)
        cell.photoView.image = UIImage(named: gem.imageName!)

        return cell
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedGem = gems[indexPath.row]
        delegate?.gemSelected(selectedGem)
